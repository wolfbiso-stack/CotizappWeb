...
const QRServiceTicket = ({ service, user, company: companyProp, onClose, darkMode }) => {
    ...
    const fetchCompany = async () => {
        try {
            // Use the passed user prop or fallback to current auth user
            ...
            <div className={`relative w-full max-w-md max-h-[90vh] flex flex-col rounded-3xl overflow-hidden shadow-2xl transition-all ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
                ...
                <div className="flex-1 overflow-y-auto bg-gray-100 min-h-0">
                    <div className="p-4 flex flex-col items-center min-h-full">
                        ...
                        <div
                            ref={ticketRef}
                            className="bg-white w-full max-w-[380px] shadow-xl overflow-hidden relative flex flex-col text-sm shrink-0"
